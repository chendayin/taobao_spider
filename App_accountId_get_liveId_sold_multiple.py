...
def getUrl(startTime, endTime, liveId, date_str, cookie, accountId):
    data = r'{"param":"{\"queryId\":\"12|2481235|undefined\",\"cubeId\":\"dws_mit_cbot_slr_lime_rpt_itm_deal\",\"queryDetail\":false,\"startTime\":\"%s\",\"endTime\":\"%s\",\"timeType\":2,\"sign\":null,\"limit\":2000,\"row\":\"[{\\\"name\\\":\\\"商品id\\\",\\\"isMeasure\\\":false}]\",\"measure\":\"[{\\\"name\\\":\\\"引导成交件数\\\",\\\"isMeasure\\\":true},{\\\"name\\\":\\\"引导成交金额\\\",\\\"isMeasure\\\":true}]\",\"column\":\"[]\",\"orders\":\"[]\",\"filter\":\"[{\\\"name\\\":\\\"开播日期分区字段yyyymmdd\\\",\\\"type\\\":\\\"dimension\\\",\\\"isMeasure\\\":false,\\\"values\\\":[\\\"%s\\\"],\\\"oper\\\":\\\"=\\\"},{\\\"name\\\":\\\"直播间id\\\",\\\"type\\\":\\\"dimension\\\",\\\"isMeasure\\\":false,\\\"values\\\":[\\\"%s\\\"],\\\"oper\\\":\\\"=\\\"},{\\\"name\\\":\\\"主播id\\\",\\\"type\\\":\\\"dimension\\\",\\\"isMeasure\\\":false,\\\"values\\\":[%s],\\\"oper\\\":\\\"=\\\"}]\",\"extra\":null}","innerId":""}' % (
...
def insert_table(accountId, liveId, itemId, sellNum, totalNum, startTime, endTime, stm):
    db = MY_DB.get_connect()
    cursor = db.cursor()
    sql = """
            replace into Anchor_get_goods_sold values('{}','{}','{}','{}','{}','{}','{}','{}')
            """.format(accountId, liveId, itemId, sellNum, totalNum, startTime, endTime, stm)
    cursor.execute(sql)
    db.commit()
    print(f"成功插入 商品 {itemId},销量 {sellNum}")
    ...
    while True:
        try:
            ...
            for c in content:
                ...
                for r in result:
                    insert_table(accountId, liveId, r[0]['raw'], r[1]['raw'], r[2]['raw'],
                                 convent_time(startTime),
